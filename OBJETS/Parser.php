...
/* http://www.phpsources.org/scripts391-PHP.htm
 * Script base sur le parser html disponible ici : http://php-html.sourceforge.net/ 
 */ 
...
class HtmlParser { 
    ...
    */ 
    ...
    function isValidTagIdentifier ($name) { 
        //return ereg ("^[A-Za-z0-9_-]+$", $name); //---- CGT : deprecated
        ...
        while ($parser->parse()) { 
            if ($parser->iNodeType == NODE_TYPE_ELEMENT) { 
                ...
                if ($cur_max == $max) { 
                    ...
                    if (($pos = strrpos(substr($parser->iNodeValue, 0,  
                        ($max - $cur_len + strlen( $separateur ))),  
                        $separateur)) !== false) { 
